...
export async function getMyDisputesApi(params?: DisputeListParams) {
  ...
  return client.get<{
    ...
    totalPages: number;
    last: boolean;
  }>(DISPUTE_ENDPOINTS.BASE, { params: queryParams });
}
...
export async function adminResolveDisputeApi(
  ...
  const client = getApiClient();
  return client.post<DisputeResult>(
    DISPUTE_ENDPOINTS.ADMIN_RESOLVE(id),
    request
  );
}
...
export async function adminGetDisputesApi(params?: DisputeListParams) {
  ...
  return client.get<{
    ...
    totalPages: number;
    last: boolean;
  }>(DISPUTE_ENDPOINTS.ADMIN_LIST, { params: queryParams });
}
