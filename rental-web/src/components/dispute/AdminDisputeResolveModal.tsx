...
export function AdminDisputeResolveModal({
  ...
  const titleId = useId();
  const [selectedType, setSelectedType] = useState<DisputeResolveType | null>(null);
  const [refundAmount, setRefundAmount] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const firstBtnRef = useRef<HTMLButtonElement>(null);

  // focus trap: 열릴 때 첫 버튼 포커스
  useEffect(() => {
    if (isOpen) {
  ...
  async function handleConfirm() {
    ...
    try {
      await adminResolveDisputeApi(dispute.id, {
        type: selectedType,
        refundAmount: selectedType === "PARTIAL" ? Number(refundAmount) : undefined,
      });
      ...
      if (err instanceof ApiRequestError) {
        ...
        const msg = err instanceof Error ? err.message : "분쟁 해결에 실패했습니다.";
        setErrorMsg(msg);
        toast.error(msg);
      }
    } finally {
