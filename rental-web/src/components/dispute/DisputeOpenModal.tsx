...
export function DisputeOpenModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApiRequestError && err.status === 409) {
        ...
        const message =
          err instanceof Error ? err.message : "분쟁 오픈에 실패했습니다.";
        setErrorMsg(message);
        toast.error(message);
      }
    } finally {
  ...
  // 이미지 첨부 (presigned URL 훅 사용 가능, 여기서는 blob URL로 stub)
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex justify-between text-xs mt-1">
              ...
              <span
                className={descLength > 1000 ? "text-red-500" : "text-gray-400"}
              >
                {descLength} / 1000
              </span>
            </div>
          ...
          <div>
            <p className="text-sm font-medium text-gray-700 mb-1">
              증빙 이미지 첨부{" "}
              <span className="text-gray-400 font-normal">(최대 5장)</span>
            </p>
            <input
              type="file"
              accept="image/*"
