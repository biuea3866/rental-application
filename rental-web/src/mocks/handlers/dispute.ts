...
export const STUB_DISPUTES: DisputeResult[] = [
  ...
  },
  {
    ...
    attachmentUrls: ["https://example.com/img2.jpg", "https://example.com/img3.jpg"],
    refundAmount: 30000,
...
/** 핸들러 간 공유 상태 */
let stubDisputes: DisputeResult[] = [...STUB_DISPUTES];
...
export const disputeHandlers = [
  ...
  http.post(`${BASE_URL}/api/v1/disputes`, async ({ request }) => {
    ...
    if (hasActiveDispute(body.rentalId)) {
      return HttpResponse.json(
        { code: "DISPUTE_ALREADY_ACTIVE", message: "이미 진행 중인 분쟁이 있습니다." },
        { status: 409 }
      );
    }
  ...
  http.post(`${BASE_URL}/api/v1/disputes/:id/cancel`, async ({ params }) => {
    await delay(200);
    const id = Number(params.id);
    const dispute = stubDisputes.find((d) => d.id === id);
    if (!dispute) {
    ...
    if (dispute.status !== "OPEN") {
      return HttpResponse.json(
        { code: "INVALID_STATUS", message: "OPEN 상태의 분쟁만 취소할 수 있습니다." },
        { status: 400 }
      );
    }
  ...
  http.patch(
    ...
    async ({ params }) => {
      ...
      if (dispute.status !== "OPEN") {
        return HttpResponse.json(
          { code: "INVALID_STATUS", message: "OPEN 상태의 분쟁만 검토 시작 가능합니다." },
          { status: 400 }
        );
      }
  ...
  http.post(
    ...
    async ({ params, request }) => {
      ...
      if (dispute.status !== "UNDER_REVIEW") {
        return HttpResponse.json(
          { code: "INVALID_STATUS", message: "UNDER_REVIEW 상태의 분쟁만 해결 가능합니다." },
          { status: 400 }
        );
      }
