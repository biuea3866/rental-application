...
const AVAILABLE_PRODUCT = STUB_PRODUCTS.find((p) => p.status === "AVAILABLE")!;
const RENTED_PRODUCT = STUB_PRODUCTS.find((p) => p.status === "RENTED")!;
...
describe("ProductDetail", () => {
  it("상품 제목이 렌더링되어야 한다", () => {
    ...
    expect(
      screen.getByText(AVAILABLE_PRODUCT.title)
    ).toBeInTheDocument();
  });
  ...
  it("가격 정보가 정상 표시되어야 한다", () => {
    ...
    const priceText = `${AVAILABLE_PRODUCT.pricePerDay.toLocaleString()}원`;
  ...
  it("보증금 정보가 표시되어야 한다", () => {
    ...
    const depositText = `${AVAILABLE_PRODUCT.deposit.toLocaleString()}원`;
  ...
  it("위치 정보가 표시되어야 한다", () => {
    ...
    expect(screen.getByText(AVAILABLE_PRODUCT.location)).toBeInTheDocument();
  });
  ...
  it("등록자 이름이 표시되어야 한다", () => {
    ...
    expect(
      screen.getByText(AVAILABLE_PRODUCT.lenderName)
    ).toBeInTheDocument();
  });
  ...
  it("대여 중인 상품에는 '대여 불가' 버튼이 비활성화되어야 한다", () => {
    renderWithQuery(<ProductDetail product={RENTED_PRODUCT} />);
