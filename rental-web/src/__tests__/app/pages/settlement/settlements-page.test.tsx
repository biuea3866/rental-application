...
import { STUB_SETTLEMENTS } from "@/mocks/handlers/settlement";
...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/mypage/settlements",
}));
...
describe("정산 내역 페이지", () => {
  ...
  it("로딩 중 스켈레톤이 표시되어야 한다", async () => {
    ...
    const SettlementsPage = (
      await import("@/app/(auth)/mypage/settlements/page")
    ).default;
    renderWithQuery(<SettlementsPage />);

    expect(screen.getByTestId("loading-state")).toBeInTheDocument();
  });

  it("정산 목록과 요약이 정상적으로 렌더링되어야 한다", async () => {
    const stubData: SettlementListResponse = {
      content: STUB_SETTLEMENTS,
      totalElements: STUB_SETTLEMENTS.length,
      totalPages: 1,
    };
    ...
    expect(
      screen.getByTestId(`settlement-card-${STUB_SETTLEMENTS[0].settlementId}`)
    ).toBeInTheDocument();
  });

  it("정산 내역이 없을 때 빈 상태가 표시되어야 한다", async () => {
    sharedMockClient.get.mockResolvedValue({
      success: true,
