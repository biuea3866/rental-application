...
describe("AdminDisputeActions", () => {
  ...
  it("OPEN 상태에서 '해결' 버튼은 렌더되지 않는다", () => {
    ...
    expect(screen.queryByRole("button", { name: /해결|전체 환불|부분 환불|기각/i })).not.toBeInTheDocument();
  });
  ...
  it("UNDER_REVIEW 상태에서 '검토 시작' 버튼이 렌더되지 않는다", () => {
    ...
    expect(screen.queryByRole("button", { name: /검토 시작/i })).not.toBeInTheDocument();
  });
  ...
  it("RESOLVED_REFUND 상태에서 액션 버튼이 렌더되지 않는다", () => {
    ...
    expect(screen.queryByRole("button", { name: /검토 시작|해결/i })).not.toBeInTheDocument();
  });
