      ...
      <div className="min-h-screen bg-gray-50">
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
            <button onClick={() => router.back()} className="p-1 rounded-full hover:bg-gray-100" aria-label="뒤로가기">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        <section className="bg-white rounded-xl p-4 shadow-sm">
          ...
          <dl className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <dd className="text-gray-900">{REASON_LABELS[dispute.reason] ?? dispute.reason}</dd>
            </div>
            <div className="flex justify-between text-sm">
              <dt className="text-gray-500">오픈일</dt>
              <dd className="text-gray-900">{new Date(dispute.createdAt).toLocaleDateString("ko-KR")}</dd>
            </div>
            {dispute.resolvedAt && (
              <div className="flex justify-between text-sm">
                <dt className="text-gray-500">해결일</dt>
                <dd className="text-gray-900">{new Date(dispute.resolvedAt).toLocaleDateString("ko-KR")}</dd>
              </div>
            )}
            {dispute.refundAmount != null && (
              <div className="flex justify-between text-sm font-semibold border-t border-gray-100 pt-2 mt-2">
                ...
                <dd className="text-green-700">{dispute.refundAmount.toLocaleString("ko-KR")}원</dd>
              </div>
            )}
          </dl>
        </section>
        ...
        {dispute.attachmentUrls.length > 0 && (
          <section className="bg-white rounded-xl p-4 shadow-sm">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">증빙 이미지</h2>
