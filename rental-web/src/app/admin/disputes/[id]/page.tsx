...
import { adminGetDisputesApi, getDisputeApi } from "@/lib/api/dispute";
...
export default function AdminDisputeDetailPage({ params }: AdminDisputeDetailPageProps) {
  ...
  // 관리자는 GET /api/v1/admin/disputes로 목록 조회하나
  // 단건은 /api/v1/disputes/{id} (ADMIN 권한이면 403 없음 — BE 설계)
  ...
  if (isError || !dispute) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => refetch()} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 mr-2">
          다시 시도
        </button>
        <button onClick={() => router.back()} className="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50">
          뒤로가기
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-gray-900">분쟁 #{dispute.id}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <section className="bg-white rounded-xl p-5 border border-gray-200">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">분쟁 정보</h2>
        ...
        <div className="space-y-4">
          <section className="bg-white rounded-xl p-5 border border-gray-200">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">관리자 액션</h2>
            <AdminDisputeActions
              dispute={dispute}
              onUpdated={() => {
